...
def plan_curved_trajectory(target_position):
    ...
    # target_position_3d = np.array([target_position[0], target_position[1], 0])

    # --------------------------- DON'T DO MATRIX MULTIPLICATION -- DO TRIG MULTIPLICATION
    # --------------------------- target_position[0] + cos() * x1 SOMETHING LIKE THIS BUT GET THETA VALUE FROM ROLL PITCH YAW --- MIGHT NEED TO SWITCH OPERATORS
    cos_y = math.cos(yaw)
    sin_y = math.sin(yaw)

    # Compute absolute target position in odom frame
    x2 = x1 + target_position[0] * cos_y - target_position[1] * sin_y ## TODO: How would you get x2 from our target position? Remember this is relative to x1
    y2 = y1 + target_position[0] * sin_y + target_position[1] * cos_y## TODO: How would you get x2 from our target position? Remember this is relative to x1
